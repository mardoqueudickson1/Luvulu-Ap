...
exports.PrismaAppointmentRepositoy = void 0;
const prisma_service_1 = require("../../../prisma/prisma.service");
const common_1 = require("@nestjs/common");
const errorsMessages_1 = require("../../../../utils/shared/errorsMessages");
let PrismaAppointmentRepositoy = class PrismaAppointmentRepositoy {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async findUser(id) {
    ...
    async findById(id) {
        ...
        if (appointment.Doctor.user.FCMToken) {
            token = appointment.Doctor.user.FCMToken[0].token;
        }
        console.log(...oo_oo(`2638949231_202_4_202_22_4`, token));
        const isReported = appointment.medicalReport && appointment.medicalReport.length > 0
            ? true
            : false;
        const isRated = appointment.Rating && appointment.Rating.length > 0 ? true : false;
        const appointments = {
            isRated: isRated,
            ...
            pacient: {
                ...
                avatar: appointment.pacient.user.avatar,
            },
            doctor: {
                doctorToken: token,
    ...
    async findByDate(date, doctorId) {
        const findAppointment = await this.prisma.appointment.findFirst({
            where: { date, doctorId },
        });
        return findAppointment;
    }
    async findAllInDayFromProvider({ doctorId, day, month, year, }) {
    ...
    async findAllInMonthFromProvider({ doctorId, month, year, }) {
        const parsedMonth = String(month).padStart(2, '0');
        const appointments = await this.prisma.appointment.findMany({
            where: {
                doctorId,
    ...
    async findAppoitmentByPatientId(patientId) {
        try {
    ...
    async findAppoitmentByDoctorId(id) {
        try {
            const appointmentsData = await this.prisma.appointment.findMany({
                where: { doctorId: id },
                include: {
    ...
    async findAppoitmentByclinicId(id) {
        try {
            const appointmentsData = await this.prisma.appointment.findMany({
            ...
            const appointments = appointmentsData.map((appointment) => ({
                ...
                institution: {
                    ...
                    user: {
                        ...
                        addressComplement: appointment.institution.user.addressComplement,
                        municipality: appointment.institution.user.municipality,
                        avatar: appointment.institution.user.avatar || null,
                    },
                },
            }));
            return appointments;
...
;
oo_te;
//# sourceMappingURL=prismaAppointmentRepository.js.map